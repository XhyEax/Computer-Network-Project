...
char listen_mac_address[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
// IP地址及协议类型(网络层)
char dst_ip[15] = "127.0.0.1";
char listen_ip[15] = "127.0.0.1";
unsigned char proto_udp = 17;

// UDP端口号(传输层)
unsigned short listen_port = 4321;
...
//解包udp
void unpack_segment(in_addr_t saddr, in_addr_t daddr, char *src_ip_str, unsigned char *segment, int segment_len)
{
    ...
    // printf("dst port: %d\n", uh_dport);
    //判断端口
    if (uh_dport == listen_port /*&& uh_sport == dst_port*/)
    {
        //保存接收到的checksum
...
void unpack_packet(unsigned char *packet, int packet_len)
{
    ...
    //检查IP地址及协议类型
    if (!strcmp(dst_ip_str, listen_ip) && /*!strcmp(dst_ip_str, dst_ip) &&*/ header.proto == proto_udp)
    {
        //校验checksum
        ...
        //解包segment
        unpack_segment(header.srcIP, header.dstIP, src_ip_str, packet + header_len, packet_len - header_len);
    }
}
...
void unpack_frame(unsigned char *frame, int frame_len)
{
    ...
    //检查目标地址、协议类型
    if (!memcmp(header.destination_address, listen_mac_address, 6) &&
        // !memcmp(header.source_address, dst_mac_address, 6) &&
...
int pack_packet(in_addr_t sipaddr, in_addr_t dipaddr, char *buffer, char *payload, int packet_payload_len)
{
    ...
    return pack_frame(dst_mac_address, listen_mac_address, protocol_type, ether_buffer, buffer, frame_payload_len);
}
...
void sender()
{
    ...
    while (1)
    {
        ...
        {
            // 层层打包
            int frame_len = pack_segment(listen_port, dst_port, udp_buffer, payload, payload_len);
...
int main()
{
    ...
    sll.sll_pkttype = PACKET_OUTGOING;           //包类型标志为本地回环
    sll.sll_halen = 6;                           //目标MAC地址长度为6
    memcpy(sll.sll_addr, listen_mac_address, 6); //监听MAC地址
    //子线程监听收到的消息
    pthread_t recv_thread;
    int recv_thrd = pthread_create(&recv_thread, NULL, (void *)&receiver, NULL);
    //打印地址
    printMacAddress("src mac address", listen_mac_address, 6);
