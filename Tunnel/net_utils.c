...
//解包udp
int unpack_segment(in_addr_t saddr_t, in_addr_t daddr_t, char *src_ip_str, unsigned char *segment, int segment_len)
{
...
//解包ip
int unpack_packet(unsigned char *packet, int packet_len)
{
    ...
    //检查IP地址及协议类型
    if (!strcmp(dst_ip_str, listen_ip) && header.proto == proto_udp)
    {
        //校验checksum
        ...
        //解包segment
        return unpack_segment(header.srcIP, header.dstIP, src_ip_str, packet + header_len, packet_len - header_len);
    }
    return -1;
}

//解包帧
int unpack_frame(char *frame, int frame_len)
{
    ...
    {
        //解包packet
        return unpack_packet(frame + frame_header_len, frame_len - frame_header_len);
    }
    return -1;
}
...
void tun_router()
{
    ...
    while ((frame_len = recvfrom(sd, frame, max_frame_size, 0, NULL, NULL)) > 0)
    {
        ...
        struct frame_header header;
        int frame_header_len = sizeof(header);
        int payload_len = unpack_frame(frame, frame_len);
        if (payload_len > 0)
        {
...
void tun_receiver()
{
    ...
    while ((frame_len = recvfrom(sd, frame, max_frame_size, 0, NULL, NULL)) > 0)
    {
        ...
        if (payload_len > 0)
        {
            // printHexData("recv ip_in_ip", payload, payload_len);
            // receiver
            payload_len = unpack_packet(payload, payload_len);
            if (payload_len > 0)
            {
...
// 设置监听信息 绑定socket
void bind_socket(char *my_src_mac_address, const char *my_src_ip, int my_src_port)
{
    //mac地址 (数据链路层)
